...
export default function EigenleistungenPage() {
  ...
  function formularAendern(gewerk: string, feld: keyof NeuesFormular, wert: string) {
    setFormulare(prev => {
      ...
      // Gesamtpreis auto-berechnen wenn Menge + Einzelpreis gefüllt
  ...
  if (positionen.length === 0) {
    return (
      <div className="text-center py-20">
        ...
        <p className="text-gray-500 mb-8">Markiere zuerst Positionen als Eigenleistung auf dem Dashboard.</p>
        <Link href="/" className="bg-blue-600 text-white px-8 py-3 rounded-xl hover:bg-blue-700 transition-colors">
          Zum Dashboard
        </Link>
      </div>
    );
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Eigenleistungs-Planer</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-5 border-l-4 border-blue-500">
          ...
          <div className={`text-2xl font-bold ${nettoErsparnis >= 0 ? 'text-blue-600 dark:text-blue-400' : 'text-red-600 dark:text-red-400'}`}>
            {formatEuro(nettoErsparnis)}
          </div>
        ...
        {gewerke.map(gewerk => {
          ...
          return (
            <div key={gewerk} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
              {/* Gewerk-Header */}
              <button
                onClick={() => setOffeneGewerke(prev => {
                  const next = new Set(prev);
                  isOffen ? next.delete(gewerk) : next.add(gewerk);
                  return next;
                })}
              ...
              >
                ...
                <div className="flex items-center gap-6 text-sm">
                  ...
                  <span className={`font-bold ${gwNetto >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-500'}`}>
                    Netto: {gwNetto >= 0 ? '+' : ''}{formatEuro(gwNetto)}
                  </span>
                </div>
              </button>
              ...
              {isOffen && (
                <div className="p-6 space-y-6">
                  ...
                  <div>
                    ...
                    {/* Formular: Neues Material */}
                    <div className="flex gap-2 items-end flex-wrap">
                      <div className="flex-1 min-w-40">
                        <label className="text-xs text-gray-400 mb-1 block">Bezeichnung</label>
                        <input
                          value={f.bezeichnung}
                          onChange={e => formularAendern(gewerk, 'bezeichnung', e.target.value)}
                          onKeyDown={e => e.key === 'Enter' && materialHinzufuegen(gewerk)}
                      ...
                      <div className="w-20">
                        ...
                        <input
                          value={f.menge}
                          onChange={e => formularAendern(gewerk, 'menge', e.target.value)}
                          placeholder="10"
                          className="w-full text-sm border border-gray-200 dark:border-gray-600 rounded-lg px-3 py-2 focus:outline-none focus:border-blue-400 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100"
                        />
                      </div>
                      <div className="w-24">
                        ...
                        <input
                          value={f.einheit}
                          onChange={e => formularAendern(gewerk, 'einheit', e.target.value)}
                          placeholder="m²"
                          className="w-full text-sm border border-gray-200 dark:border-gray-600 rounded-lg px-3 py-2 focus:outline-none focus:border-blue-400 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100"
                        />
                      </div>
                      <div className="w-28">
                        ...
                        <input
                          value={f.einzelpreis}
                          onChange={e => formularAendern(gewerk, 'einzelpreis', e.target.value)}
                          placeholder="25,00"
                          className="w-full text-sm border border-gray-200 dark:border-gray-600 rounded-lg px-3 py-2 focus:outline-none focus:border-blue-400 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100"
                        />
                      </div>
                      <div className="w-28">
                        ...
                        <input
                          value={f.gesamtpreis}
                          onChange={e => formularAendern(gewerk, 'gesamtpreis', e.target.value)}
                          placeholder="250,00"
                          className="w-full text-sm border border-gray-200 dark:border-gray-600 rounded-lg px-3 py-2 focus:outline-none focus:border-blue-400 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100"
                        />
                      </div>
                      <button
                        onClick={() => materialHinzufuegen(gewerk)}
                        disabled={speichernLaden === gewerk || !f.bezeichnung.trim()}
                    ...
                    <div className="flex justify-end">
                      <div className="bg-gray-50 dark:bg-gray-700 rounded-lg px-5 py-3 text-sm space-y-1 min-w-64">
                        ...
                        <div className="flex justify-between font-bold border-t border-gray-200 dark:border-gray-600 pt-1 mt-1">
                          ...
                          <span className={gwNetto >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-500'}>
                            {formatEuro(gwNetto)}
                          </span>
                        </div>
                      </div>
